...
public class AggTradeListToProfitTest {
	...
	public void aggTradeListToProfitWithSellListTest() throws Exception {
		...
		Assert.assertEquals(new BigDecimal("9.3"), profit.getQuantity());
		Assert.assertEquals(BaseCurrency.BTC, profit.getBaseCurrency());
		Assert.assertEquals(new BigDecimal("0.00403687"), profit.getProfitValue());
		Assert.assertEquals(new BigDecimal("-0.013842784"), profit.getPriceDifference());
		Assert.assertEquals(sell3.getTradeTime().getTime() - buy.getTradeTime().getTime(), profit.getTimeDifference());
	}
	...
	public void aggTradeListToProfitWithBuyListTest() throws Exception {
		...
		Assert.assertTrue(profit != null);
		Assert.assertEquals(new BigDecimal("5"), profit.getQuantity());
		Assert.assertEquals(BaseCurrency.BTC, profit.getBaseCurrency());
		Assert.assertEquals(new BigDecimal("16"), profit.getPriceDifference());
		Assert.assertEquals(new BigDecimal("-13"), profit.getProfitValue());
		Assert.assertEquals(sell.getTradeTime().getTime() - buy1.getTradeTime().getTime(), profit.getTimeDifference());
	}
