...
public class AggregatedTradeServiceImpl implements AggregatedTradeService {
	...
	var tradeToDelete = aggTradeRepository.findById(id);
	...
	return aggTradeRepository.findAllWhereProfitIsNull();
    }
	...
	for (Integer integer : ids) {
	    var trade = getById((long) integer);
	    if (trade != null) {
		trades.add(trade);
	    } else {
	...
	if (!trades.stream().anyMatch(AggregatedTrade::isBuy)) {
	...
	var buy = trades.stream().filter(AggregatedTrade::isBuy).findFirst().get();
	var sell = trades.stream().filter(x -> !x.isBuy()).findFirst().get();
	try {
	...
	return trades.stream().sorted(Comparator.comparing(AggregatedTrade::isBuy).reversed())
